...
// 写入编码前缀，其中smalltag=0xC0,largetag=0xF7
func puthead(buf []byte, smalltag, largetag byte, size uint64) int {
	if size < 56 {
		// 如果长度为0-55个字节，则前缀为 0xC0 + 长度值
...
// intsize返回存储i所需的最小字节数
func intsize(i uint64) (size int) {
	for size = 1; ; size++ {
		// 一个字节是256，此处使用了 >>= 左移 运算符
		if i >>= 8; i == 0 {
			return size
		}
	}
